...
describe('SkillSelector', () => {
  ...
  it('groups skills by category', () => {
    render(<SkillSelector allSkills={mockSkills} selectedSkillIds={[]} onChange={() => {}} />);
  ...
  it('shows skill count in category header', () => {
    render(<SkillSelector allSkills={mockSkills} selectedSkillIds={[1, 2]} onChange={() => {}} />);
  ...
  it('calls onChange when skill is toggled', () => {
    ...
    render(<SkillSelector allSkills={mockSkills} selectedSkillIds={[]} onChange={handleChange} />);
  ...
  it('removes skill when unchecking selected skill', () => {
    const handleChange = vi.fn();
    render(
      <SkillSelector allSkills={mockSkills} selectedSkillIds={[1, 2]} onChange={handleChange} />
    );
  ...
  it('shows checked state for selected skills', () => {
    render(<SkillSelector allSkills={mockSkills} selectedSkillIds={[1, 4]} onChange={() => {}} />);

    const reactCheckbox = screen.getByRole('checkbox', { name: /React/i }) as HTMLInputElement;
    const leadershipCheckbox = screen.getByRole('checkbox', {
      name: /Leadership/i,
    }) as HTMLInputElement;
    const teachingCheckbox = screen.getByRole('checkbox', {
      name: /Teaching/i,
    }) as HTMLInputElement;
  ...
  it('allows removing skills from summary', () => {
    const handleChange = vi.fn();
    render(
      <SkillSelector allSkills={mockSkills} selectedSkillIds={[1, 4]} onChange={handleChange} />
    );
